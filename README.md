...
В части **View** представлен абстрактный класс `Component<T>`, который реализуется в нескольких классах - `Page`, `Modal`, `ItemCard`, `Form<T>`, `Basket`, `Tabs`, `Success`. Классы осуществляют создание визуального образа приложения в виде различных компонентов.
...
Класс `ItemCard<T>` наследует от класса `Component<T>` и представляет себя визуальную реализацию индивидульной карточки товара. Это дженерик класс, приниающий определенный тип данных T. Имеет следующие поля:
...
Класс `Tabs` наследует `Component<T>` и реализует функционал выбора опции путем переключения между кнопками (используется в форме заказа для выбора способа оплаты). Класс имеет поле:
