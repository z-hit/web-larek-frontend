...
В части **Controller** представлен класс **EventEmitter**, который отслеживает изменения состояния приложения (события), вносит изменения в данные Модели и оповещает о них необходимые компоненты приложения, которые меняются в соответствии с произошедшим событием. Взаимодействие происходит по принципу promise-based flow.
...
2. **Api**
   Класс Api реализует взаимодействие с Сервером и имеет следующие поля:
...
1. **Model<T>**
   В части Модели данных представлен абстрактный дженерик класс Model. Он принимает в конструктор constructor(data: Partial<T>, protected events: IEvents) определенный тип данных T и список событий, которые добавляет в создаваемый объект.
...
2. AppState
   Класс AppState реализует класс Model и сожержит в себе ключевые данные проекта, а именно поля:
...
- preview: string | null - статус попапа с карточкой.
...
3. **ItemCard<T>**
   Класс ItemCard наследует от класса Component и представляет себя визуальную реализацию индивидульной карточки товара. Это дженерик класс, приниающий определенный тип данных T. Имеет следующие поля:
...
- \_index?: HTMLElement - счетчик позиции товара в списке (опционально);
...
a. **ItemCardCatalog**
Класс ItemCardCatalog наследует от класса ItemCard и является визуальным представлением карточки товара в каталоге на главной странице. В данном классе конструктор визуально реализует определенные родительские поля - title, price, image, category.

b. **ItemCardPreview**
Класс ItemCardPreview наследует от класса ItemCard и является визуальным представлением карточки товара в модальном окне просмотра товара. В данном классе конструктор визуально реализует определенные родительские поля - title, price, image, category, description, button.

c. **ItemCardBasket**
Класс ItemCardBasket наследует от класса ItemCard и является визуальным представлением карточки товара в списке товаров в Корзине. В данном классе конструктор визуально реализует определенные родительские поля - counter, title, price, button.

4. **Form<T>**
   Класс Form<T> наследует от класса Component и является визуальным представлением формы для ввода информации пользователя о заказе. Это дженерик класс, который принимает определенный типа T.
...
a. **Order**
Класс Order наследует от класса Form и визуально реализует элемент заполнения формы заказа с указанием способа оплаты и адреса доставки.

Конструктор класса constructor(container: HTMLFormElement, events: IEvents) принимает элемент контейнер и события и создает форму через родительский конструктор, добавляя туда кнопки выбора способа оплаты класса Tabs и поле ввода адреса.
...
5. **Modal**
   Класс Modal наследует от Component и является визуальной реализацией элемента модального окна, отрываемого для просмотра товара, просмотра корзины и оформления заказа.
...
7. **Basket**
   Класс Basket наследует от Component и является визуальным представлением открытой корзины с товарами, добавленными пользователем.

   Класс имеет поля:

- items: HTMLElement[] - коллекция карточек товаров, добавленных пользователем;
- selected: string[] - маркер для отметки того, есть ли в корзине товары(для активации кнопки);
...
8. **Tabs**
   Класс Tabs наследует Component и реализует функционал выбора опции путем переключения между кнопками (используется в форме заказа для выбора способа оплаты). Класс имеет поле:
...
type FormErrors = Partial<Record<keyof IOrder, string>> // _обект с ошибками заполнения формы_
type ItemID = string // _id товара_
type Payment = 'Онлайн' | 'При получении' // _варианты оплаты_
type Category = // _варианты категорий товаров_
| 'другое'
| 'софт-скилл'
| 'дополнительное'
| 'кнопка'
...
// _состояние кнопки переключения_
type TabState = {
selected: string;
}

// _действия с кнопкой переключения_
type TabActions = {
...
type EventName = string | RegExp // _название события_
type Subscriber = Function // _типа колбека для событий - функция_
...
// _типы событий и их интерфейсы_
interface IEvents {
...
- items:changed - изменили список товаров (в каталоге)
- modal:open - открыть модального окна
- modal:close - закрыть модального окна
- order:open - открыть формы заполнения данных о заказе
- contacts:open - открыть формы заполнения данных о контактах
- order:submit - отправить данных о заказе (по нажатию книпки)
- contacts:submit - отправить контактных данных (по нажатию кнопки)
- formErrors:change - измененилось состояние ошибок заполнения формы
- /^order\..\_:change/ - измененяется состояния формы заказа (при заполнении)
- /^contacts\..\_:change/ - измененяется состояния формы контактов (при заполнении)
- basket:open - открыть корзины
- basket:changed - изменилось состояния корзины (списка добавленнызх товаров, суммы)
- card:select - выбрать карточку (открыть модальное окно с карточкой)
- preview:changed - изменилось модальное окна с карточкой
