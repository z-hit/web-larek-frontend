...
a. Класс ItemCardCatalog наследует от класса ItemCard и является визуальным представлением карточки товара в каталоге на главной странице. В данном классе визуально реализованы родительские поля title, price, image, category.

b. Класс ItemCardPreview наследует от класса ItemCard и является визуальным представлением карточки товара в модальном окне просмотра товара. В данном классе визуально реализованы родительские поля title, price, image, category, description, button.

c. Класс ItemCardBasket наследует от класса ItemCard и является визуальным представлением карточки товара в списке товаров в Корзине. В данном классе визуально реализованы родительские поля counter, title, price, button.
...
render(state: Partial<T> & IFormState) - принимает объект с состоянием и возвращает готовый элемент формы.
